...
- **authorId is stored directly on Post.** Each post has exactly one author, 
so no join table is needed; a join table would only be justified if posts could have multiple authors.
